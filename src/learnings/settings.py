...
# Three levels up because this module lives at src/learnings/settings.py:
# settings.py -> learnings/ -> src/ -> repo root. Getting this wrong is quiet
# and dangerous — pydantic-settings does not error on a missing env_file, it
# just leaves every field at its default, jwt_secret included.
_ENV_FILE = Path(__file__).resolve().parent.parent.parent / ".env"

# Sentinel default for ``Settings.jwt_secret``; api/app.py treats it as fatal.
...
class Settings(BaseSettings):
    ...
    # Kept as a placeholder rather than a required field so local development
    # and tests work with no configuration. api/app.py refuses to start when
    # it is still this value — anyone holding it can forge a dashboard session
    # for any user, so it must never reach a deployment.
